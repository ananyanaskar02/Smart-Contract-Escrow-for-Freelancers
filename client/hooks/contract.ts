...
export const CONTRACT_ADDRESS =
  "CBWYMJB5KHP45DKCN6XW7ROMAMG7JWIIMNRG3U4A773MLDB5A4U6DE6C";
 ...
 * Calls: init(client: Address, freelancer: Address, token: Address, amount: i128)
 */
 ...
 * Calls: release()
 */
export async function releaseFunds(caller: string) {
  return callContract("release", [], caller, true);
}
 ...
 * Calls: refund()
 */
export async function refundEscrow(caller: string) {
  return callContract("refund", [], caller, true);
}

/**
 * Check escrow status (read-only).
 * Calls: status() -> Symbol
 * Returns: "PEND" (pending) or "DONE" (released)
 */
export async function getEscrowStatus(caller?: string) {
  return readContract("status", [], caller);
}
